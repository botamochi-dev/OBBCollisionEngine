...
function #math:float
execute positioned 0 0 0 run summon minecraft:armor_stand ~-0.5 ~ ~-0.5 {Tags:["sin"]}
data modify entity @e[tag=sin,limit=1] Rotation[0] set from storage math: out.add

#* Entity を ^ ^ ^1 へテレポート
execute as @e[tag=sin] at @s run tp ^ ^ ^1

#* EntityのPos[2](Z座標を取得)
execute store result storage calc: out float 0.000001 run data get entity @e[tag=sin,limit=1] Pos[2] 1000000
