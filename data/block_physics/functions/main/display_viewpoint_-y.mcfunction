###* ブロックディスプレイ面上のプレイヤーの視線の位置を計算
#? 従来のディスプレイの視線の向きからローカル座標系のXYZベクトルを取得する方法では、ブロックディスプレイがZ軸回転をする場合に有効ではなかった。
#? そのため、ブロックディスプレイのクォータニオンからローカル座標系のXYZベクトルを取得し、視点の位置を計算する
...
#* ディスプレイ面上の座標系におけるXYZ軸方向の単位ベクトルを求める
# ディスプレイのPos取得　#! ブロックディスプレイのPosではなく面上に配置したディスプレイのPosであることに注意
data modify storage block_physics: display_Pos set from entity @s Pos
# ブロックディスプレイのクォータニオン取得 #!display_set ですでに取得済み
#block_physics: block_quaternion
# ブロックディスプレイの共役クォータニオン計算 #!display_set ですでに取得済み
#block_physics: conj_quaternion
...
execute store result score $x1 number run data get storage block_physics: display_Pos[0] 10000
...
execute store result score $y1 number run data get storage block_physics: display_Pos[1] 10000
...
execute store result score $z1 number run data get storage block_physics: display_Pos[2] 10000
...
#計算結果を適用
summon minecraft:area_effect_cloud ^ ^ ^ {Tags:["vector_xd"]}
execute store result entity @e[tag=vector_xd,limit=1] Pos[0] double 0.0001 run scoreboard players get $x1 number
execute store result entity @e[tag=vector_xd,limit=1] Pos[1] double 0.0001 run scoreboard players get $y1 number
execute store result entity @e[tag=vector_xd,limit=1] Pos[2] double 0.0001 run scoreboard players get $z1 number
# ディスプレイとの差を計算
...
scoreboard players operation $z1 number += $z2 number
#計算結果を適用
summon minecraft:area_effect_cloud ^ ^ ^ {Tags:["vector_yd"]}
execute store result entity @e[tag=vector_yd,limit=1] Pos[0] double 0.0001 run scoreboard players get $x1 number
execute store result entity @e[tag=vector_yd,limit=1] Pos[1] double 0.0001 run scoreboard players get $y1 number
execute store result entity @e[tag=vector_yd,limit=1] Pos[2] double 0.0001 run scoreboard players get $z1 number
# ディスプレイとの差を計算
...
scoreboard players operation $z1 number += $z2 number
#計算結果を適用
summon minecraft:area_effect_cloud ^ ^ ^ {Tags:["vector_zd"]}
execute store result entity @e[tag=vector_zd,limit=1] Pos[0] double 0.0001 run scoreboard players get $x1 number
execute store result entity @e[tag=vector_zd,limit=1] Pos[1] double 0.0001 run scoreboard players get $y1 number
execute store result entity @e[tag=vector_zd,limit=1] Pos[2] double 0.0001 run scoreboard players get $z1 number
# ディスプレイとの差を計算
...
#* デバッグ用
#execute if entity @e[tag=rotation_display_click] as @e[tag=vector_xd] at @s run particle dust 1 0 0 1 ~ ~ ~ 0 0 0 0 10
#execute if entity @e[tag=rotation_display_click] as @e[tag=vector_yd] at @s run particle dust 0 1 0.216 1 ~ ~ ~ 0 0 0 0 10
#execute if entity @e[tag=rotation_display_click] as @e[tag=vector_zd] at @s run particle dust 0 0.235 1 1 ~ ~ ~ 0 0 0 0 10
